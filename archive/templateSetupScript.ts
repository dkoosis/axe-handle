...
const requiredStructure: Record<string, Record<string, string[] | Record<string, string[]>>> = {
  'express': {
    'server': ['server.ejs'],
    'handler': ['handler.ejs'],
    'types': ['types.ejs'],
...
async function ensureDir(dirPath: string): Promise<void> {
  try {
    ...
    console.log(chalk.green(`✓ Directory exists: ${dirPath}`));
  } catch (error) {
...
async function ensureTemplateFile(filePath: string, content: string): Promise<void> {
  try {
    ...
    console.log(chalk.green(`✓ Template exists: ${filePath}`));
  } catch (error) {
    await fs.writeFile(filePath, content || '// Template content goes here');
    console.log(chalk.blue(`Created template: ${filePath}`));
  }
}
...
async function setupTemplates(): Promise<void> {
  console.log(chalk.cyan('Setting up template directory structure...'));
